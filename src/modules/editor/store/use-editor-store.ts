...
interface EditorStore {
  ...
  getTabs: (projectId: Id<"projects">) => TabState;
  openFile: (
...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  openFile: (projectId, fileId, { pinned }) => {
    ...
    // case 1 -  when no file is pinned, replace the tab with the new one
    if (!isOpen && !pinned) {
      const newTab = previewTabId
    ...
    // case 2 - when the file is pinned (double click the file), open a new tab
    if (!isOpen && pinned) {
      tabs.set(projectId, {
        ...state,
    ...
    // case 3 - toggle pinned (add or remove to preview) when the file is already open
